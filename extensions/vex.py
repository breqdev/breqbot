...
class Vex(Breqcog):
    ...
    async def teaminfo(self, ctx, team: str):
        ...
        async with ctx.channel.typing():
            ...
            embed = discord.Embed(title=f"{team['program']} team {team['number']}: {team['team_name']}")

            awards_raw = requests.get("https://api.vexdb.io/v1/get_awards",
                                  params={"team": team["number"],
                                          "season": self.SEASON}).json()["result"]

            awards = []
            for award_raw in awards_raw:
                award_name = award_raw["name"]

                event_raw = requests.get("https://api.vexdb.io/v1/get_events",
                                         params={"sku": award_raw["sku"]}).json()["result"][0]
                event_name = event_raw["name"]
                awards.append((award_name, event_name))

            awards = "\n".join(f"• **{award}** | {event}" for award, event in awards)
            if not awards:
                awards = "This team has not won any awards."

            embed.add_field(name="Awards", value=awards, inline=False)

            rankings_raw = requests.get("https://api.vexdb.io/v1/get_rankings",
                                        params={"team": team["number"],
                                                "season": self.SEASON}).json()["result"]
            ...
            for ranking_raw in rankings_raw:
                ...
                teams_count = requests.get("https://api.vexdb.io/v1/get_teams",
                                           params={"sku": ranking_raw["sku"],
                                                   "nodata": "true"}).json()["size"]
