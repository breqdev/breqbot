...
class QueryHandler:
    ...
    def start(self):
        self.thread = socketio.start_background_task(self.forward_queries)

    def forward_queries(self):
        pubsub = redis_client.pubsub()
        pubsub.subscribe("portal:query")
        ...
        while True:
            ...
            if message["portal"] not in sids:
                continue

            emit("portal_query", message, room=sids[message["portal"]])
...
@socketio.on("portal_register")
def handle_portal_register(data):
    print("Registration received")
    portal_id = data["portal"]
    name = data["name"]
    desc = data["desc"]
    redis_client.hset(f"portal:{portal_id}", mapping={"id": portal_id, "name": name, "desc": desc})
    redis_client.sadd("portal:list", portal_id)
    sids[portal_id] = request.sid
    portals[request.sid] = portal_id
    print("Portal registered")

@socketio.on("portal_completed")
def handle_portal_completed(data):
    print("Completed job received")
    job_id = data["job"]
    result = data["result"]
    portal_id = portals[request.sid]
    redis_client.publish(f"portal:{portal_id}:{job_id}", result)
    print("Completed job forwarded")

@socketio.on("disconnect")
def handle_disconnect():
    print("Disconnection received")
    try:
        portal_id = portals[request.sid]
    except KeyError:
        pass
    else:
        redis_client.srem("portal:list", portal_id)
        redis_client.delete(f"portal:{portal_id}")
        try:
            del sids[portal_id]
            del portals[request.sid]
        except KeyError:
            pass
    print("Portal deregistered")
